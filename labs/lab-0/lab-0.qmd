...
- Problematic names: Potentially "Engine size", "Fuel type", and "Year of manufacture" because they have spaces in between.
...
**Your answer:** Tibble only shows the first few lines of data and the data frame is printing the entirety of the data.
...
**Your answer:** "Year of manufacture" needs ticks because there are spaces in the name.
...
car_data <- mutate(car_data, mileage_per_year = Mileage)
...
car_data <- mutate(car_data, price_category =
                    case_when(
                    Price < 15000 ~ "budget",
...
less_30000 <- filter(car_data, Mileage < 30000)
less_30000

# Find luxury cars (from price category) with low mileage
luxury_cars <- filter(car_data, price_category == "luxury", Mileage < 30000)
luxury_cars
...
filter(car_data, Fuel == "Diesel", Age < 10)

```

**Question:** How many diesel cars are less than 10 years old?

**Your answer:** 2,040 diesel cars.
