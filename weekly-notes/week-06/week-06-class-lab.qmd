...
`r scales::dollar(base_intercept, accuracy = 1)` +
`r scales::dollar(base_slope, accuracy = 1)` × LivingArea
Interpretation:

Base price: `r scales::dollar(base_intercept, accuracy = 1)` Each sq ft
adds: `r scales::dollar(base_slope, accuracy = 1)`
:::

::: {.column width="50%"}
🏛️ Wealthy Areas Equation: Price =
`r scales::dollar(wealthy_intercept, accuracy = 1)` +
`r scales::dollar(wealthy_slope, accuracy = 1)` × LivingArea
Interpretation:

Base price: `r scales::dollar(wealthy_intercept, accuracy = 1)` Each sq
ft adds: `r scales::dollar(wealthy_slope, accuracy = 1)`
:::
:::::

::: callout-important
The Interaction Effect Wealthy areas value each sq ft
`r scales::dollar(wealthy_slope_add, accuracy = 1)` more than
non-wealthy areas!
:::
...
`r round(r2_improvement, 4)` (a `r round(r2_pct_improvement, 1)`%
relative improvement)

Interpretation: We explain `r round(r2_improvement * 100, 2)`% more
...
-   Luxury market: Every sq ft is premium
    (\$`r round(wealthy_slope, 0)`/sq ft)
-   Standard market: Space valued, but lower premium
    (\$`r round(base_slope, 0)`/sq ft)
...
`r scales::dollar(coefs_quad[1], accuracy = 1)` +
