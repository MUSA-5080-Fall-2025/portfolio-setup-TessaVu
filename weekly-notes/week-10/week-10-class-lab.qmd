...
**Behind the scenes:** We work with **log-odds**, not probabilities
directly
...
**Predictors:** - Number of exclamation marks - Contains word "free" -
Email length
...
-   `exclamation_marks`: Each additional ! multiplies odds of spam by
    `r round(odds_ratios[2], 2)`
-   `contains_free`: Having "free" multiplies odds by
    `r round(odds_ratios[3], 2)`\
-   `length`: Each additional character multiplies odds by
    `r round(odds_ratios[4], 4)` (shorter = more likely spam)
...
**This is where logistic regression gets interesting (and
complicated):**

The model gives us probabilities, but we need to make **binary
decisions**.
...
**The rest of today:** How do we evaluate these predictions and choose
thresholds?
...
But policy decisions require binary choices: spam/not spam,
approve/deny, intervene/don't intervene.
...
**Remember:** The model predicts probabilities. WE choose the threshold
that converts probabilities to yes/no predictions.
...
From the confusion matrix, we derive metrics that emphasize different
trade-offs:

**Sensitivity (Recall, True Positive Rate):**
$$\text{Sensitivity} = \frac{TP}{TP + FN}$$ *"Of all actual positives,
how many did we catch?"*

**Specificity (True Negative Rate):**
$$\text{Specificity} = \frac{TN}{TN + FP}$$ *"Of all actual negatives,
how many did we correctly identify?"*

**Precision (Positive Predictive Value):**
$$\text{Precision} = \frac{TP}{TP + FP}$$ *"Of all our positive
predictions, how many were correct?"*
...
**Remember:** The model gives us probabilities. We decide what
probability triggers action.

**Threshold = 0.3** (low bar) - More emails marked as spam - Higher
sensitivity (catch more spam) - Lower specificity (more false alarms)

**Threshold = 0.7** (high bar) - Fewer emails marked as spam - Lower
sensitivity (miss some spam) - Higher specificity (fewer false alarms)

**There is no "right" answer - it depends on the costs of each type of
error**
...
-   **Decision depends on:** Cost of intervention vs. cost of missed
    case
...
**A model can be "accurate" overall but perform very differently across
groups**
...
**COMPAS:** Commercial algorithm used in criminal justice to predict
recidivism
...
-   Black defendants twice as likely to be incorrectly labeled "high
    risk"
...
-   True Positive: Treatment cost \$1000, prevent \$50,000 in
    complications
...
*Note: This assumes we can quantify all costs, which is often impossible
for justice/equity concerns*
...
7.  **Provide uncertainty** - confidence intervals, not just point
    estimates
8.  **Enable recourse** - can predictions be challenged?

**Most importantly: Be transparent about limitations and potential
harms**
